...
%         vnewX = (accel)*1E-14 + voldX;
        angleRad = 2*pi*rand(NumElec,1);
        velocityX(Escat) = initialRV(Escat).*cos(angleRad(Escat)) + (accelx(Escat)*1E-14);
        velocityY(Escat) = initialRV(Escat).*sin(angleRad(Escat)) + (accely(Escat)*1E-14);
  ...
  %For Non-Scattering Particles
        velocityX(NoScat) = velocityX(NoScat) + accelx(NoScat)*1e-14;
        velocityY(NoScat) = velocityY(NoScat) + accely(NoScat)*1e-14;
