...
% % Vy = 2;
% Efieldx = Ex; %E=v/m
% Forcex = Efieldx*q;%Using field from b)
% Efieldy = Ey; %Using field from b)
% Forcey = Efieldy*q;
% Vavg = 0;
% 
% 
...
% %         vnewX = (accel)*1E-14 + voldX;
%         angleRad = 2*pi*rand(NumElec,1);
%         accelOnEX = newX/0.05E-7;
%         accelOnEY = newY/0.05E-7;
%         velocityX(Escat) = initialRV(Escat).*cos(angleRad(Escat)) + (accelOnEX(Escat)*1E-14);
%         velocityY(Escat) = initialRV(Escat).*sin(angleRad(Escat)) + (accelOnEY(Escat)*1E-14);
%         
%          %Find new positions
%          newX = initialX + velocityX*1e-14;
%          newY = initialY + velocityY*1e-14;
...
%         accelOnEX = newX/0.05E-7;
%         accelOnEY = newY/0.05E-7;
%         velocityX(NoScat) = velocityX(NoScat) + (accelOnEX(NoScat)*1E-14);
%         velocityY(NoScat) = velocityY(NoScat) + (accelOnEY(NoScat)*1E-14);
%          
% %         velocityX(NoScat) = velocityX(NoScat) + accelx(round(accelOnEX(NoScat)),round(accelOnEY(NoScat)))*1e-14;
% %         velocityY(NoScat) = velocityY(NoScat) + accely(round(accelOnEX(NoScat)),round(accelOnEY(NoScat)))*1e-14;
% %          %Find new positions
...
% %     figure(5)
% %     title('Electron Density')
% %     xlabel('X Distance In Plane')
% %     ylabel('Y Distance In Plane')
% %     surf(X,Y,ED)
% %  
% %     notTemp = isnan(Temp);
% %     Temp(notTemp) = 0;
